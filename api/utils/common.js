...
export function changeCase(input) {
  ...
  const joinWords = (arr, separator) => arr.join(separator);
  const titleCase = (arr) => arr.map(capitalize).join(' ');
  /* prettier-ignore */
  return {
...
export function getPrimaryKey({ tableName, schemaInfo }) {
...
export function consolidateInterfaces(interfaces) {
...
export const generateModelImports = (schemaInfo) => {
...
export function determineSQLDatabaseType(dbConnection) {
  const dbType = extractDBConnectionInfo(dbConnection).dbType;
  return dbType;
}
export const quoteTableName = (tableName) => {
...
const getTypeMappingValue = (mappings, key, columnType) => {
...
export const getTypeMapping = (column, columnType) => {
...
export const generateColumnDefinition = ({ columnName, columnType }) => {
...
export function addPrimaryKeys(schema) {
  const newSchema = {};
