...
export const getRequiredColumns = (columns) =>
  columns
    .filter((column) => column.is_nullable === 'NO')
    .map((column) => column.column_name);
export const getForeignTables = (columns) =>
  Array.from(
    new Set(
      columns
...
export const getForeignKeys = (columns) => {
  return Array.from(
    new Set(
      columns
...
export const convertTable = (table) => {
  let tableName;
  let columns;
  if (isITable(table)) {
...
export const populateChildTables = (tableMap) => {
...
const convertIntrospectedStructure = (tables) => {
