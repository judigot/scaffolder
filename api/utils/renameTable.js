function renameTable({ oldTableName, newTableName, schemaInfo }) {
  /* Helper functions for table name replacement */
  const replaceOldTableNameInTableName = (tableName) => {
  ...
  const replaceOldTableNameInColumnInfo = (columns, tableName, isPivot) => {
    return columns.map((column) => {
  ...
  const replaceOldTableNameInForeignTables = (tables) => {
    return tables.map((table) =>
      table === oldTableName ? newTableName : table,
    );
  };
  const replaceOldTableNameInForeignKeys = (keys) => {
    return keys.map((key) =>
      key === `${oldTableName}_id` ? `${newTableName}_id` : key,
    );
  };
  const replaceOldTableNameInChildTables = (tables) => {
    return tables.map((table) =>
  ...
  const replaceOldTableNameInHasOne = (relations) => {
    return relations.map((relation) =>
      relation === oldTableName ? newTableName : relation,
    );
  };
  const replaceOldTableNameInHasMany = (relations) => {
    return relations.map((relation) =>
  ...
  const replaceOldTableNameInBelongsTo = (relations) => {
    return relations.map((relation) =>
      relation === oldTableName ? newTableName : relation,
    );
  };
  const replaceOldTableNameInBelongsToMany = (relations) => {
    return relations.map((relation) =>
      relation.includes(oldTableName)
        ? relation.replace(oldTableName, newTableName)
        : relation,
  ...
  const replaceOldTableNameInPivotRelationships = (pivots) => {
