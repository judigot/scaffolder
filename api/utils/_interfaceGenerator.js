 ...
 */
export function summarizeArrayOfObjectValueTypes(data) {
 ...
 */
export function haveSimilarObjects(arr) {
  if (arr.length <= 1) {
    return true;
  }
  ...
  const compareStructure = (base, target) => {
...
export function generateInterface({
  ...
  const findOptionalKeys = (objects) => {
  ...
  const generateInterfaceContent = (obj, optionalKeys) => {
    const getArrayElementType = (arr) => {
      if (arr.length === 0) {
        return 'unknown';
      }
