 ...
 */
const parseCreateBaseMethodFileOptions = (command) => {
 ...
 */
const extractTemplateFileName = (templatePath) => {
 ...
 */
const extractFileNameReplacements = (files) => {
  const replacements = {};
  for (const file of files) {
    ...
    replacements[`${baseKey}pascalcasesingular`] =
...
export const createBaseMethodFile = (
  ...
  const result = [];
  const processedFiles = new Set(); // Track file names in lowercase for case-insensitive deduplication
  ...
  if (isScoped === true && table && schemaInfoParsed) {
    const rawTableReplacements = getReplacementsForTable(
      table,
      schemaInfoParsed,
    );
  ...
  if (recursiveWildcardMatch) {
    ...
    if (folderMatch) {
      ...
      if (templateContent.length > 0) {
        ...
        const outputFileName = applyReplacements(
          outputFilePattern,
  ...
  const uniqueFolderPathsProcessed = new Set();
  for (const folder of foldersToProcess) {
    ...
    const templateFile = files.find(
      (file) => file.name.toLowerCase() === templateFileName,
    );
    if (!templateFile) {
      continue;
    }
