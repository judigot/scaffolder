...
export const processCommand = (
  ...
  result = result.replace(useConstantRegex, (_match, group1) => {
    if (!table) {
      return '';
    }
  ...
  result = result.replace(useFormDataRegex, (_match, group1) => {
    if (!formData) {
      return '';
    }
  ...
  result = result.replace(useUserEnvRegex, (_match, group1) => {
    if (!userMetadata) {
      return '';
    }
  ...
  result = result.replace(lineWithLoopRegex, (fullLine, _newlineChar) => {
    if (!table) {
      return '';
    }
  ...
  result = result.replace(iterateRegex, (fullMatch, group1, group2) => {
    if (!table) {
      return '';
    }
    const whitespace = /^\s*/.exec(fullMatch)?.[0] ?? '';
