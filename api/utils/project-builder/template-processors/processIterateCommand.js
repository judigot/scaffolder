 ...
 */
const findFolderByPath = (path, userFiles) => {
 ...
 */
const folderContainsYamlFiles = (folder) => {
...
export const processLoopTables = (
  ...
  return content.replace(LOOP_TABLES_REGEX, (_match, options) => {
    // Parse options
...
export const processIterateCommand = (
  ...
  const shouldIgnoreFolder = (folderPath) => {
  ...
  const applyFilter = (values) => {
    ...
    const flattenedFilterList = filterList.reduce((acc, filterPattern) => {
  ...
  const allPlaceholderValues = new Map();
