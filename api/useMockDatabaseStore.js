 ...
 */
export const useMockDatabaseStore = create()((set, get) => ({
  userFiles: [],
  projects: [],
  ...
  setUserFiles: (userFiles) => {
    ...
    const constantsFolder = userFiles.find(
      (item) => item.name === 'Constants' && item.type === 'folder',
    );
    let typeMappings;
    let dbTypes;
    if (constantsFolder) {
      const typeMappingsFile = constantsFolder.children.find(
        (child) => child.type === 'file' && child.name === 'typeMappings.yaml',
      );
      if (typeMappingsFile && typeMappingsFile.content.trim() !== '') {
        try {
          const parsedData = yaml.parse(typeMappingsFile.content);
      ...
      const dbTypesFile = constantsFolder.children.find(
        (child) => child.type === 'file' && child.name === 'dbTypes.yaml',
      );
      if (dbTypesFile && dbTypesFile.content.trim() !== '') {
        try {
          const parsedData = yaml.parse(dbTypesFile.content);
          if (Array.isArray(parsedData)) {
            dbTypes = parsedData.filter((item) => typeof item === 'string');
          }
        } catch (error) {
    ...
    const projectsFolder = userFiles.find(
      (item) => item.name === 'Projects' && item.type === 'folder',
    );
    ...
    for (const folder of projectFolders) {
      ...
      const structureFile = folder.children.find(
        (child) => child.type === 'file' && child.name === 'structure.yaml',
      );
      if (structureFile) {
    ...
    const yamlFiles = projectsFolder.children.filter(
      (child) => child.type === 'file' && child.name.endsWith('.yaml'),
    );
