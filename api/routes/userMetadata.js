...
import { getUserMetadata, updateUserMetadata } from '../services/auth0Service';
...
const isEnvVariablePayload = (val) => {
  if (typeof val !== 'object' || val === null) {
    return false;
  }
  return 'envVariables' in val;
};
router.post('/user-metadata/env', (req, res) => {
  void (async () => {
    try {
      const auth0UserId =
        'auth0UserId' in req && typeof req.auth0UserId === 'string'
      ...
      const envRecord = req.body.envVariables.reduce((acc, item) => {
        if (
