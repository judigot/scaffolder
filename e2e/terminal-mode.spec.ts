...
test.describe('Terminal Mode', () => {
  test.describe('Navigation', () => {
    test('should navigate to Terminal tab from bottom navigation', async ({
      page,
      mockTerminalApi: _mockTerminalApi,
    }) => {
      await page.goto('/');
  ...
  test.describe('UI Elements', () => {
    test.beforeEach(async ({ page, mockTerminalApi: _mockTerminalApi }) => {
      await page.goto('/');
      await page.waitForLoadState('networkidle');
    ...
    test('should display action buttons', async ({ page }) => {
      ...
      await expect(
        page
          .locator(
            'button[aria-label*="yes" i], button[aria-label*="confirm" i]',
          )
          .first(),
      ).toBeVisible();
      // NO button (X)
      await expect(
        page
          .locator('button[aria-label*="no" i], button[aria-label*="cancel" i]')
          .first(),
      ).toBeVisible();
    });
    ...
    test('should display command input', async ({ page }) => {
      const input = page
        .locator(
          'input[placeholder*="terminal" i], textarea[placeholder*="terminal" i]',
        )
        .first();
    ...
    test('should display mode indicator', async ({ page }) => {
      await expect(
        page.locator('button[aria-label*="Terminal mode" i]'),
      ).toBeVisible();
      await expect(
        page.locator('button[aria-label*="Agent mode" i]'),
      ).toBeVisible();
      await expect(
        page.locator('button[aria-label*="Ask mode" i]'),
      ).toBeVisible();
    });
  });
  ...
  test.describe('Command Execution', () => {
    ...
    test('should execute pwd command and show output', async ({ page }) => {
      ...
      await expect(page.locator('text="/home/ec2-user"').first()).toBeVisible({
        timeout: 5000,
      });
    });
    ...
    test('should execute ls command and show output', async ({ page }) => {
      ...
      await expect(page.locator('text=".bashrc"').first()).toBeVisible({
        timeout: 5000,
      });
    });
    ...
    test('should handle failed commands gracefully', async ({ page }) => {
      ...
      await expect(page.locator('text="No such file"').first()).toBeVisible({
        timeout: 5000,
      });
    });
  });
  ...
  test.describe('Secondary Tabs', () => {
    ...
    test('should return to Terminal tab', async ({ page }) => {
      ...
      await expect(
        page.locator('[role="application"][aria-label="Terminal"]'),
      ).toBeVisible();
    });
  });

  test.describe('Connection Status', () => {
    test('should show connected status with host', async ({
      page,
      mockTerminalApi: _mockTerminalApi,
    }) => {
      await page.goto('/');
