...
3) Modular test suites for common “enterprise + insurance” backend requirements.
...
### 5.1 “Contract-driven suites”
Each suite:
- reads required config fields
- runs black-box HTTP assertions
- exposes clear failure messages: “expected Set-Cookie to include HttpOnly” etc.
...
Allow custom “service adapters” if an org has multiple API styles.
...
- `withFreshClient()` utility to test “no cookie” scenarios
...
Do not let `.env` become “the real config.” If it changes the meaning of the standard, it belongs in TypeScript config.
...
- Error messages must be explicit:
  - “Missing TEST_EMAIL. Provide it via .env or environment variables.”
