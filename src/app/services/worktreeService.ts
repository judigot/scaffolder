...
export async function createWorktree(
	...
	await writeChatMetadata(worktreePath, initialMetadata);
...
export async function validateWorktreeResult(
	...
	if (needsRename) {
		...
		try {
			...
			const finalWorktreePath = newWorktreePath || worktreePath;
			const existingMetadata = await readChatMetadata(finalWorktreePath);
			if (existingMetadata) {
				...
				await writeChatMetadata(finalWorktreePath, updatedMetadata);
			}
...
async function readChatMetadata(
	...
	try {
		...
		const metadata = JSON.parse(content) as ChatMetadata;
		return metadata;
	} catch {
 ...
 */
export async function writeChatMetadata(
	worktreePath: string,
...
export async function addMessageToMetadata(
	...
	const metadata = await readChatMetadata(worktreePath);
	if (!metadata) {
	...
	await writeChatMetadata(worktreePath, metadata);
}
