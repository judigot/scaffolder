...
export async function createWorktree(
  ...
  if (createResult.exitCode !== 0) {
    throw new Error(
      `Failed to create worktree: ${createResult.stderr !== '' ? createResult.stderr : createResult.stdout}`,
    );
  }
...
export async function validateWorktreeResult(
  ...
  if (needsRename) {
    ...
    try {
      ...
      const filesChanged =
        filesChangedResult.exitCode === 0 && filesChangedResult.stdout !== ''
...
> {
  ...
  for (const entry of entries) {
    ...
    if (metadata) {
      // If metadata has no branch, try to get it from git
      if (metadata.branch === '') {
        try {
          ...
          if (
            ...
            metadata.branch = branchResult.stdout.trim();
          }
        } catch {
          // Ignore errors, keep empty branch
        }
      }
