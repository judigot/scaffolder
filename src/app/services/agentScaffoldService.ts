...
export async function scaffoldToArtifact(
  ...
  try {
    manifest = selectAgentScaffoldManifest(generated.manifest, request.files);
  } catch (error: unknown) {
    if (error instanceof AgentScaffoldFileSelectionError) {
      throw new AgentScaffoldError(error.message, {
        ...
        code: error.code,
        details:
          error.selector === undefined ? undefined : { selector: error.selector },
      });
    }
    throw error;
  }
  ...
  return {
    body:
      request.output === 'zip'
        ? createAgentScaffoldZip(manifest)
        : createAgentScaffoldShell(manifest),
    contentType:
      request.output === 'zip'
        ? 'application/zip'
