...
function SchemaSelector() {
	...
	return (
		<div className="bg-bg-emphasis/50 rounded-lg border border-border p-4 mb-4">
			<div className="flex flex-col sm:flex-row sm:items-center gap-4">
				...
				<div className="flex items-end gap-2">
					...
					<button
						...
						className={`flex items-center gap-1 px-3 py-2 rounded-md text-sm font-medium transition-colors duration-200 ${
							isCurrentSchemaDirty && !isSaving && !isDeleting
								...
								: "bg-secondary text-fg-subtle cursor-not-allowed"
						}`}
					>
					...
					<button
						...
						className="flex items-center gap-1 px-3 py-2 bg-info-600 hover:bg-info-700 text-white rounded-md text-sm font-medium transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
					>
					...
					<button
						...
						className="flex items-center gap-1 px-3 py-2 bg-primary-600 hover:bg-primary-700 text-white rounded-md text-sm font-medium transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
					>
			...
			{/* Error Messages */}
			{saveError !== null && (
				<div className="mt-3 p-2 bg-danger-500/10 border border-danger-500/30 rounded-md">
					<p className="text-sm text-danger-400">Save Error: {saveError}</p>
				</div>
			)}
			{deleteError !== null && (
				<div className="mt-3 p-2 bg-danger-500/10 border border-danger-500/30 rounded-md">
					<p className="text-sm text-danger-400">Delete Error: {deleteError}</p>
				</div>
			)}
			...
			<div className="mt-3 text-xs text-fg-muted">
				{selectedSchemaName === null ? (
					...
					<span>
						File:{" "}
						<code className="text-fg">Schemas/{selectedSchemaName}.json</code>
					</span>
				)}
			</div>
		</div>
