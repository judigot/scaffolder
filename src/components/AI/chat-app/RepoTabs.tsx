...
export default function RepoTabs({
  ...
  const getRepoCount = (repo: IRepository) => {
    const filteredChats = filterChatsWithContent(repo.chats);
    const filteredSprints = filterSprintsWithContent(repo.sprints);
    return (
      filteredSprints.reduce(
        (total, sprint) => total + sprint.chats.length,
        0,
  ...
  const normalizeRepoInput = (value: string): string | null => {
    ...
    const shortOwner = shortMatch?.[1];
    const shortRepo = shortMatch?.[2];
    if (
      ...
      return `https://github.com/${shortOwner}/${shortRepo}`;
    }
  ...
  return (
    <>
      <div className="bg-secondary border-b border-border flex items-center">
        ...
        <div
          ref={containerRef}
          className="flex-1 min-w-0 px-3 py-2 flex gap-2 overflow-x-auto scrollbar-thin"
        >
        ...
        <div className="flex-shrink-0 pl-2 pr-3 py-2 bg-secondary border-l border-border/50">
          <button
            ...
            className="flex items-center gap-2 px-3 py-2 rounded-xl border border-dashed border-border text-fg-muted hover:border-primary-600/40 hover:text-fg hover:bg-primary-900/20 transition-colors"
