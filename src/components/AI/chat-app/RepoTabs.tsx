...
export default function RepoTabs({
  ...
  const [dropdownPosition, setDropdownPosition] = useState<IDropdownPosition>({ top: 0, left: 0 });
  ...
  const handleAddRepo = async () => {
    ...
    const exists = repositories.some((r) => r.repoUrl.toLowerCase() === trimmedUrl.toLowerCase());
    if (exists) {
  ...
  return (
    <>
      <div ref={containerRef} className="px-3 py-2 bg-secondary border-b border-border flex gap-2 overflow-x-auto scrollbar-thin">
            ...
            <div key={repo.id} className="relative flex-shrink-0">
              ...
              >
                ...
                >
                  ...
                  <span className="text-sm font-semibold max-w-[120px] truncate">{repo.name}</span>
                ...
                <button
                  ...
                  className={`h-full px-2 rounded-r-xl ${
                    openRepoId === repo.id ? 'bg-secondary-hover text-fg' : 'text-fg-subtle'
                  }`}
                  onClick={(event) => {
                    event.stopPropagation();
                    setOpenRepoId((prev) => (prev === repo.id ? null : repo.id));
                  }}
      ...
      {openRepoId !== null && openRepo !== undefined && createPortal(
        <div
        ...
        >
          <div className="space-y-3">
            <div className="space-y-2">
              <p className="text-[10px] uppercase tracking-wide text-fg-subtle">Regular Chats</p>
              ...
              {openRepo.chats.map((chat) => (
                ...
                >
                  ...
                  {chat.prStatus !== null && (
                    <Badge label={chat.prStatus === 'ready' ? 'Ready' : 'Draft'} variant={chat.prStatus === 'ready' ? 'ready' : 'draft'} />
                  )}
                </button>
              ))}
            </div>
            <div className="border-t border-border pt-3 space-y-3">
              <p className="text-[10px] uppercase tracking-wide text-fg-subtle">Sprint Chats</p>
              ...
              {openRepo.sprints.map((sprint) => (
                <div key={sprint.id} className="space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-semibold text-fg">{sprint.name}</span>
                    <Badge
                      label={sprint.status}
                      variant={
                  ...
                  {sprint.chats.map((chat) => (
                    <button
                      ...
                      className="w-full flex items-center justify-between gap-2 px-3 py-2 rounded-xl bg-secondary text-left text-sm text-fg-muted hover:bg-secondary-hover hover:text-fg transition-colors"
                      onClick={() => {
                        onSelectBranch(openRepo.id, chat.id, 'sprint', sprint.id);
                        setOpenRepoId(null);
                      }}
                    >
                      <span className="truncate font-medium">{chat.title}</span>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60">
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <h2 id="add-repo-title" className="text-lg font-semibold text-fg">Add Repository</h2>
              <button
                type="button"
                onClick={() => {
                  setShowAddModal(false);
            ...
            <div className="space-y-4">
              <div>
                <label htmlFor="repo-url" className="block text-sm font-medium text-fg-muted mb-2">
