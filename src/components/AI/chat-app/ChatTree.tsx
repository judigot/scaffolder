...
export default function ChatTree({
  ...
  const sprintOpenState = useMemo(() => {
    if (Object.keys(openSprints).length > 0) {return openSprints;}
  ...
  const renderChatRow = (chat: IChat, scope: 'sprint' | 'regular', onSelect: () => void) => {
    const messagesLength = chat.messages.length;
    const lastMessage = messagesLength > 0 ? chat.messages[messagesLength - 1] : null;
    ...
    return (
      ...
      >
        <div className="flex items-center justify-between gap-2">
          ...
          <span className="text-xs text-fg-subtle">{formatRelativeTime(chat.updatedAt)}</span>
        </div>
        ...
        <div className="mt-2 flex items-center gap-2">
          <span className="text-[11px] text-fg-subtle">{chat.messages.length} msgs</span>
          {chat.prStatus && (
  ...
  return (
    <div className="w-full md:w-[360px] bg-bg border-b md:border-b-0 md:border-r border-border h-full flex flex-col">
      <div className="px-4 py-3 flex items-center justify-between border-b border-border">
        <div>
          <p className="text-xs uppercase tracking-wide text-fg-subtle">Chats</p>
      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin px-3 py-3 space-y-3">
        <button
          ...
          onClick={() => { setIsRegularOpen((prev) => !prev); }}
        >
          <div className="flex items-center gap-2">
            <span className={`transition-transform ${isRegularOpen ? 'rotate-90' : ''}`}>
        ...
        <div className={isRegularOpen ? 'space-y-2' : 'hidden'}>
          {regularChats.length === 0 && (
            <div className="px-3 py-3 text-xs text-fg-subtle">No regular chats</div>
          )}
          {regularChats.map((chat) =>
            renderChatRow(chat, 'regular', () => { onSelectRegularChat(chat.id); })
          )}
        </div>
        ...
        {sprints.map((sprint) => (
          <div key={sprint.id} className="space-y-2">
            ...
            >
              <div className="flex items-center gap-2">
                <span className={`transition-transform ${sprintOpenState[sprint.id] ? 'rotate-90' : ''}`}>
                  <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
                    <title>Toggle</title>
                    <path d="M8.59 16.59L13.17 12 8.59 7.41 10 6l6 6-6 6z" />
                  </svg>
              ...
              <div className="flex items-center gap-2">
                ...
                />
                <span className="text-xs text-fg-subtle">{sprint.chats.length}</span>
              </div>
            </button>

            <div className={sprintOpenState[sprint.id] ? 'space-y-2' : 'hidden'}>
              {sprint.chats.length === 0 && (
                <div className="px-3 py-3 text-xs text-fg-subtle">No sprint chats</div>
              )}
              {sprint.chats.map((chat) =>
                renderChatRow(chat, 'sprint', () => { onSelectChat(chat.id); })
              )}
            </div>
          </div>
        ))}
