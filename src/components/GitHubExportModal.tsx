...
interface IGitHubExportModalProps {
  ...
  owner: string;
  accessToken: string;
  onSelectMethod: (method: 'personal_token' | 'github_app' | 'existing_repo', tokenOrRepoUrl?: string) => void;
...
function GitHubExportModal({
  ...
  const [selectedMethod, setSelectedMethod] = useState<'personal_token' | 'github_app' | 'existing_repo' | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchOptions = async () => {
      ...
      try {
        const response = await fetch(`${getApiUrl()}/check-github-export-options`, {
          method: 'POST',
          headers: {
  ...
  const renderOptionCard = (option: IExportOption) => {
    ...
    const isExistingRepoSelectable = isExistingRepo && option.action?.type === 'needs_repo_url';
    ...
    return (
      <div
        ...
        `}
        onClick={() => {
          if (option.available && (!isExistingRepo || isExistingRepoSelectable)) {
            setSelectedMethod(option.method);
          }
        }}
        onKeyDown={(e) => {
          if (e.key === 'Enter' && option.available && (!isExistingRepo || isExistingRepoSelectable)) {
            setSelectedMethod(option.method);
          }
        }}
        role="button"
        tabIndex={option.available && (!isExistingRepo || isExistingRepoSelectable) ? 0 : -1}
      >
        {/* Recommended badge */}
        {option.recommended && (
        ...
        <div className="grid grid-cols-2 gap-3 mt-4">
          <div>
            <p className="text-xs font-medium text-emerald-400 mb-1.5 uppercase tracking-wide">Advantages</p>
          ...
          <div>
            <p className="text-xs font-medium text-rose-400 mb-1.5 uppercase tracking-wide">Considerations</p>
            <ul className="space-y-1">
            ...
            {option.action.type === 'needs_install' && (
              <div className="space-y-3">
                ...
                {isWaitingForInstall ? (
                  ...
                  <button
                    type="button"
                    onClick={() => { handleInstallApp(option.action?.url ?? ''); }}
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-gray-500">
                    Enter the URL of an existing repository you want to push files to
                  </p>
                </div>
  ...
  const canProceed = () => {
    if (selectedMethod === 'personal_token') {
      const patOption = options?.options.find((o) => o.method === 'personal_token');
      return patOption?.action?.type === 'has_token' || tokenInput.trim() !== '';
    }
    ...
    if (selectedMethod === 'existing_repo') {
      ...
      const existingRepoOption = options?.options.find((o) => o.method === 'existing_repo');
      return existingRepoOption?.action?.type === 'needs_repo_url' && isValidGitHubRepoUrl(repoUrlInput);
    }
    return false;
  };
  ...
  return (
    <CustomModal isOpen={isOpen} onClose={onClose} title="Export to GitHub" size="large">
      <div className="min-h-[400px]">
        {isLoading && (
          <div className="flex flex-col items-center justify-center h-64 gap-4">
            <div className="w-10 h-10 border-3 border-blue-500 border-t-transparent rounded-full animate-spin" />
            <p className="text-gray-400">Checking your GitHub configuration...</p>
          </div>
        )}
            ...
            <div className="flex items-center gap-3 p-4 bg-gray-800/50 rounded-lg border border-gray-700">
              ...
              <div>
                ...
                <p className="text-sm text-gray-400">
                  {options.ownerType === 'Organization' ? 'Organization' : 'Personal Account'}
              ...
              <div className="p-4 bg-blue-500/10 border border-blue-500/30 rounded-lg">
                <div className="flex gap-3">
                  ...
                  <div className="text-sm text-blue-300">
                    <p className="font-medium mb-1">Why do personal accounts need a token?</p>
                    <p className="text-blue-400/80">
                      GitHub Apps can only create repositories in <strong>organizations</strong>, not personal accounts.
                      For personal accounts, you need a Personal Access Token to create repositories.
                    </p>
                  </div>
                </div>
              </div>
