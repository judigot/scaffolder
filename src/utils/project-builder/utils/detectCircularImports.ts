...
function extractImportPaths(content: string): string[] {
  ...
  try {
    const parsedYaml: unknown = parse(content);
    
    // Extract imports from root level keys
    if (isRecord(parsedYaml)) {
      // Handle root level import keys (IMPORT_PROJECT(path):)
      const rootImportKeys = Object.keys(parsedYaml)
        .filter(key => key.startsWith(PROJECT_ACTIONS.IMPORT_PROJECT));
      ...
      const processNode = (node: unknown): void => {
        if (Array.isArray(node)) {
          ...
          for (const item of node) {
            if (typeof item === 'string' && item.startsWith(`${PROJECT_ACTIONS.IMPORT_PROJECT}(`)) {
              const importPathRegex = /IMPORT_PROJECT\((.*?)\)$/;
    ...
    console.error('Error parsing YAML to extract imports:', String(error));
  }
