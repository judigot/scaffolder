...
const fetchGitHubFiles = async (
	...
	// In dev mode (isUsingLocalFiles), we don't need a URL - fetch from local /files
	// In production, we need a valid repo URL
	if (!isUsingLocalFiles && !params.publicRepoURL) {
		// Return empty structure if no repo URL provided (production only)
		return [];
	}
