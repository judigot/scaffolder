...
import { db } from '../db';
import { user, session, oauthAccount } from '../db/schema';
...
app.post('/register', zValidator('json', registerSchema), async (c) => {
  ...
  // Fetch created user
  const [createdUser] = await db.select().from(user).where(eq(user.id, userId));
...
app.post('/login', zValidator('json', loginSchema), async (c) => {
  ...
  // Create session
  const { session: newSession, sessionCookie } = await createSession(
    existingUser.id,
  );
  setSessionCookie(c, sessionCookie);
...
// GET /auth/session - Check session status (for AuthContext)
...
app.get('/oauth/:provider', async (c) => {
  ...
  if ('createAuthorizationURL' in provider) {
    if (providerName === 'github') {
      ...
      url = (provider as any).createAuthorizationURL(
        state,
        codeVerifier,
        scopes,
      );
...
app.get('/oauth/:provider/callback', async (c) => {
  ...
  try {
    ...
    if (providerName === 'github') {
      ...
      tokens = await (provider as any).validateAuthorizationCode(
        code,
        codeVerifier,
      );
    }
    ...
    const [existingOAuthAccount] = await db
      ...
      .where(
        and(
          eq(oauthAccount.providerId, providerName),
          eq(oauthAccount.providerUserId, oauthUserInfo.id),
        ),
      )
      .limit(1);
    ...
    if (existingOAuthAccount) {
      ...
      const [existingUser] = await db
        .select()
        .from(user)
        .where(eq(user.email, oauthUserInfo.email))
        .limit(1);
      ...
      if (existingUser) {
        ...
        const username = generateUsername(
          oauthUserInfo.name || oauthUserInfo.email,
        );
        ...
        await db.insert(user).values({
          ...
          username: username,
          firstName: oauthUserInfo.name?.split(' ')[0] || null,
          lastName: oauthUserInfo.name?.split(' ').slice(1).join(' ') || null,
