 ...
 */
describe('UserType API - CRUD Testing (C→R→U→R→D→R)', () => {
  ...
  it('should CREATE a new user_type', async () => {
    const res = await fetch(`${BASE_URL}/api/user-type`, {
      method: 'POST',
  ...
  it('should READ the created user_type', async () => {
    const res = await fetch(`${BASE_URL}/api/user-type/${createdId}`);
  ...
  it('should UPDATE the user_type', async () => {
    const res = await fetch(`${BASE_URL}/api/user-type/${createdId}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
  ...
  it('should READ the updated user_type', async () => {
    const res = await fetch(`${BASE_URL}/api/user-type/${createdId}`);
    expect(res.status).toBe(200);
  });

  // 5. DELETE - Delete the record
  it('should DELETE the user_type', async () => {
    const res = await fetch(`${BASE_URL}/api/user-type/${createdId}`, {
      method: 'DELETE',
    });
    expect(res.status).toBe(200);
